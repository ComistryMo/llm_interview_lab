...
选择岗位、求职阶段、难度与面试官模式。系统先确定面试流程并一次展示一个问题；连接普通 LLM API 或 Codex 后，AI 只根据岗位技能、难度、求职意向和你明确授权的材料生成当前第一问，读取回答后再逐步追问，不会在开场生成整场计划。代码题由本地 Grader 判定并仍来自已验证 Catalog，非代码题按 Rubric 记录回答证据。结束后报告保存在当前学习档案，且不会修改 Practice mastery。Codex 使用官方 App Server 的结构化 `outputSchema`，并沿用你在“设置 → 模型与推理强度”中保存的模型和推理强度。
