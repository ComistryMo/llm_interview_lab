...
非代码面试还支持“开始录音 → 停止 → 转录 → 编辑 → 提交”的可选流程。当前源码默认提供 **SenseVoiceSmall 本地转录**：不需要 API Key、PyTorch 或独立服务器，首次在语音区域点击下载约 240 MB 模型，之后可离线使用。识别结果追加到回答框、保留已有文字，不自动提交。详见[本地语音转文字](local-stt.md)。
