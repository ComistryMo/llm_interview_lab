...
作答页采用上方题目、下方一体式回答区的布局。回答区随内容增高，到达上限后独立滚动；光标移动和持续输入会带动回答滚动，不会把“提交并继续”挤出窗口。标题旁不再显示多余的“详情”；模型连接与材料授权收进回答区，底部“结束本场”仍需确认。准备页按“面试目标 / 面试官与背景”分组，小窗口可以滚动查看其余设置，“开始面试”固定在底部。

动态面试不要求先填自评分数；固定面试的自评与人工证据入口仍保留。回答锁定后不可改写，新的问题使用空白回答框，已提交的回答仍保存在原题。输入框获焦时提示立即隐藏，中文输入法组字时也不会与提示叠在一起。动态回答区的“语音”可展开现有录音与转录选项。
