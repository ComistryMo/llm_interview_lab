...
作答页采用上方题目、下方一体式回答区的布局。回答区随内容增高，到达上限后独立滚动；光标移动和持续输入会带动回答滚动，不会把“提交并继续”挤出窗口。顶部“详情”查看本场岗位、难度与状态，底部“结束本场”仍需确认。准备页按“面试目标 / 面试官与背景”分组，小窗口可以滚动查看其余设置，“开始面试”固定在底部。
...
Codex 与普通 API 使用同一套逐轮流程及本地结果校验。每次提交都重新校验授权范围与材料 SHA；Codex 使用新的传输 Thread 并显式带上本场历史，避免继承其他场次或本轮取消授权的材料。它仍是连续的一问一答，不依赖 Codex Thread 充当学习记录。Codex 请求使用官方 `outputSchema`，范围来自本轮实际允许的阶段、岗位技能和本地代码候选；返回内容仍须通过本地校验才保存。普通 API 保留 JSON 提示与同一套本地校验，不声称所有 Provider 都有 Schema 保证。下一问在完整响应校验后展示，不把尚未通过校验的半段 JSON 当成题目。
