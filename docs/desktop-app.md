...
当前转录仅接入 OpenAI / OpenAI-compatible 的 `/audio/transcriptions`，使用独立的 `whisper-1` 模型，所选服务必须支持这个接口。DeepSeek 文字面试连接不会再被列为转录服务。转录连接选择可以记住，但每次新录音或换题后都需要重新勾选远程转录授权；转录失败不删除本地音频。此轮未调用真实远程转录服务。
