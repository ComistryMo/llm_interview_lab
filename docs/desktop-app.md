...
逐轮请求包含目标岗位的技能说明、求职阶段、难度要求、本场已发生的问答、当前回答与本次授权材料。设置页可同时选择简历与一份 JD / 补充经历材料。模型先听经历介绍，再从多个角度追深；一个方向讲清后转向其他有背景依据的方向，不应整场只重复一个关键词。项目之后明确进入原理/八股，要求至少三个相关主题，困难至少四个；同一主题的补充追问不能代替其他主题。之后从当前可运行、已验证的本地题目中选择一题手撕。只保存已经展示的问题，不预生成未来题单。没有可运行代码题时保留缺失环节，不能标记整场完整完成。
...
手撕环节先显示题面，点击底部「开始作答」进入编辑器，「查看题面」可随时返回，代码不会清空。动态面试按「运行代码」或 `Ctrl+R` / `Command+R` 会先保存当前文本，再在本机执行脚本，显示 stdout、错误、退出码与 revision；单次运行最多 30 秒。「公开测试」是独立的可选动作，仍运行原有题库单元测试。自测退出码 0 只表示脚本正常退出，不代表算法正确或公开测试通过。

点击「提交给面试官」会锁定当前代码、该 revision 的自测输入/输出及公开测试事实，复用本场 Codex/API 连接进行主观评价；不要求代码完整或测试通过，也不会自动补全代码。失败后点击「重试代码评价」，重用原快照；锁定后不能改写证据。修改后没有重跑的旧结果会标记为本版本未运行，不发送成新代码的执行证据。评分完成后点击「结束并查看复盘」。历史固定面试的 Grader 记录入口及其客观评分规则保持兼容，已冻结的旧问题/Rubric 不重写；完整新体验请新开一场。

本轮验收建议：继续使用原数据目录，**新开一场**困难面试，依次检查「自我介绍 → 多角度经历深挖 → 四个相关原理主题 → 手撕自测 → AI 代码评价 → 复盘」。手撕时可先运行自己构造的样例，再提交未完成代码，检查系统是否将核心逻辑、实际运行和公开测试分开记录。模型的语义深度仍需实际判断，最低轮数不能保证每个主题足够不同。[实现与验证记录](../plans/active/adaptive-interview-depth-and-coding.zh.md) 区分了真实 Python/QML 测试、模拟 AI 回复和真实模型抽查。
...
策略实际由 [`dynamic-interviewer.md`](../coach/prompts/dynamic-interviewer.md) 和 [`interview_flow.py`](../src/llm_interview_lab/interview_flow.py) 发送给 Codex 与普通 API，不只是维护者文档。已经取消每阶段四问和旧追加路径二十问上限。简单/标准/困难的经历最低覆盖为 4/6/8 轮，原理为 3/3/4 轮；达到最低数后仍可继续追问，真实面试计时限制整场时长。策略要求简单至少两个技术角度、标准两到三个、困难至少三个，并在困难档深入机制/数学关系、实现、反例和实验检验；评分锚点不因难度改变宽容度。

代码的最低轮数门控防止直接跳过环节，不等于能证明模型确实问到了不同主题或足够深入。模型还需根据完整已发生对话、岗位技能和简历/JD选择问题；对语义质量的观察不作所有模型都必然遵循的保证。提前结束或时间不足仍标记未完成，不伪称完整。
...
非代码面试支持“语音输入 → 边说边显示文字 → 完成录音、补齐尾句 → 编辑 → 提交”。当前源码默认使用 **Zipformer 本地流式中英转录**，SenseVoice 已移除。无需 API Key、PyTorch 或服务器，首次在「语音设置」下载约 199 MB，之后离线复用。实时文字先在回答框上方显示，完成后追加到草稿，保留手打内容，不自动提交。本机 `workspace/maintainer/manual-uat` 已准备并校验新模型，重启源码应用即可使用。详见[本地流式语音输入](local-stt.md)。
