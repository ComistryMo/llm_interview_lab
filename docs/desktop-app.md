...
在已保存的连接上点击「编辑」，页面会定位到配置表单并选中模型字段；修改后点击「保存并测试」。只有通过实际连接测试的连接才使用绿色就绪状态，尚未测试不会显示为已连接。

同一次运行中，添加材料或刷新同一档案不再清空未改变配置的成功测试状态；重新保存连接（包括替换 Key）或切换档案后，需重新测试。这个状态是本机最近的测试结果，不是持续网络可用性的保证。
