...
选择岗位、求职阶段、难度与面试官模式。系统冻结面试蓝图并一次展示一个问题；代码题由本地 Grader 判定，非代码题按 Rubric 记录回答证据。连接普通 LLM API 后，可在首版 Golden Path（后训练工程师 / 校招 / 标准难度）中预览并确认由 AI 根据已授权材料生成的非代码问题；Coding 题仍由已验证 Catalog 决定。结束后报告保存在当前学习档案，且不会修改 Practice mastery。
