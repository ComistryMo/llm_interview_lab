...
当前实现仍是“当前题目 + 当前回答 → 一条 AI 追问”，不是已完成的全流程自适应面试引擎：请求包含岗位、级别、难度、当前技能标识、题目、评分要求和本次授权材料，但尚未统一组装整场历史对话与完整岗位技能说明，也不会自动从动态追问切换到手撕环节。固定面试中的代码题仍来自已验证 Catalog，由本地 Grader 判定；模拟面试结果不会修改 Practice mastery。Codex 使用官方 App Server 的结构化 `outputSchema`，并沿用“设置 → 模型与推理强度”的配置。
