...
状态：布局、输入与候选技术门禁通过，已纳入 Alpha.4 发布。以下为正式 `AppController` / `Main.qml` 的隔离合成档案，不是 Phase 0 原型。画面中的未完成面试和草稿真实持久化；没有真实模型、麦克风或私人简历传输，不能以截图证明 AI 已连通。平台包和未验收事项见[候选报告](../desktop-candidate-20260909-report.zh.md)及[发布说明](../release-notes-v0.4.0-alpha.4.md)；原始截图与来源 SHA 不回写。
