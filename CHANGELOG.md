...
**当前回到源码预发布迭代阶段，不以稳定版口径承诺质量。** 已发布的 v1.0.0 和更早记录作为历史事实保留，不重新编号或删除。

## [未发布]

### 1.0.1-alpha.1：源码快速迭代、连接诊断与应用内更新

- 分阶段连接错误、脱敏诊断复制及 Key 写入后读回校验。
- 基于 SHA-256 的文件/数据块复用，设置内准备更新、确认安装和重启；保留失败回退。
- 发布门禁要求两平台增量资源并验证重建，不退回手动下载完整安装包。
- 新增 `scripts/run_desktop.py`：一次准备环境，此后直接运行当前源码；修改后重启，不编译安装包，不清空数据。
- 根据用户最新决定不打包、不发布；应用内增量安装仍待平台产物验收。详见 [v1.0.1-alpha.1 说明](docs/release-notes-v1.0.1-alpha.1.md)。
...
[0.1.0]: https://github.com/ComistryMo/llm_interview_lab/tree/v0.1.0
