...
> 历史归档（2026-09-09），不作为当前待执行指令。新的动态面试语义以[统一难度计划](../active/unified-difficulty-interview-workbench.zh.md)与当前发布说明为准，原始失败及阶段结果保留。

## 2026-09-07：面试界面视觉收敛（当前切片）

- 基线 `c904b3e`；用户要求继续改善布局与美观，保留已经跑通的一次提交、授权、AI、计时和评分语义。
- 实际问题：状态和标题重复；回答框与提交分离；多行底部操作挤占题目；复盘卡片套卡片。采用有限阅读宽度、固定一体式回答区、轻量页头、单一主动作、无嵌套边框的证据列表。侧栏只微调信息层级。
- 只改相关 QML 与现有薄 Token、直接交互测试和说明；不改 Controller / Domain，不调用云模型、不新增框架、不构建、不触发 CI、不跑全量。
- 先保存正式页面 Before；实现后检查 900×620 / 1080×680 / 1280×800 / 1440×900、深浅主题、100% / 125% 字号，实际输入、提交、停止、长文本滚动和复盘。证据只用隔离合成档案，位于 ignored `workspace/maintainer/interview-polish-20260907/`。
- 收尾要求：至少两轮真实截图查看；局部测试通过后提交、推送并给出 Before/After，不将 UI 测试误称为再次验证真实云服务。
- 已完成正式页面交互与多轮实看：统一阅读宽度、固定自适应回答区、精简准备页和复盘、细滚动条与侧栏层级。长回答光标跟随、语音展开定位的迭代缺陷均直接复现并修复；集成 22 项交互及后续受影响子集、2 项既有契约通过。详情与历史失败记录见同目录状态报告。终局 `UI_POLISH_READY_FOR_USER_UAT`，不启动下一阶段或发布。

## 2026-09-07：一次提交接续面试（历史切片）

- 用户指出“保存回答 → 让 Codex 继续 → 再确认发送”反直觉，并要求真实跑通。基线 `d7570dd4fd778a3a999f8425e72fef1a67a0b0e9`，继续 `fix/dynamic-interview-full-flow-20260905`。
- 当前决策替代旧的逐轮确认操作：开场一次列出本场背景、材料和问答发送范围；用户每次点击“提交并继续”，保存后自动请求下一问。只有范围变化或旧会话未授权时重新确认；不允许隐式读取新材料。
- 修改限于既有 InterviewPage、Controller 局部、直接测试和本计划/状态报告/桌面指南；不修改 Domain 阶段、题库、评分、Grader、Provider 架构。
- 新入口 `submitInterviewAnswer` 复用现有保存和 AI 请求方法。重复点击被门控；失败保留已锁定回答，重试不重写；未连接时自动建立面试官连接并发送。QSettings 仅保存按场次绑定的授权指纹和所选连接，不保存材料正文或 Key。
- 为 Codex 本轮响应传入官方 `outputSchema`，只允许当前真实阶段、技能和代码候选；本地校验仍负责是否落盘。失败在面试页显示具体下一步和操作编号，不再只转成全局“操作未完成”。
- 验证：先复现旧正式界面的多次确认，再用 Windows 正式 QML、真实 Codex 与全新合成简历/JD跑完整场次。直接测试覆盖一次点击、重试、连接、授权变化与四尺寸；禁止全量、CI、打包。
- 证据目录 `workspace/maintainer/interview-submit-20260907/`（ignored）。不读取或关闭用户原有 UAT 会话；不把测试探针或假模型结果当真实服务证据。结果与未解决网络延迟记录在同目录的状态报告。
- 完成：真实 Codex `gpt-5.6-sol / low` 依次完成五次口述提交，动态进入 FND-002 手撕；真实 Grader 记录 starter 失败后，六题形成 `completed` 报告，重启恢复六题/证据/授权/偏好通过。定向集成 18 项、错误补验 1 项、既有兼容 2 项通过。终局 `REAL_CODEX_SINGLE_SUBMIT_FLOW_COMPLETED / WAITING_FOR_USER_UAT`；网络重连使每次模型请求仍需约 125–128 秒，不宣称已优化完延迟。

## 2026-09-07：真实交互收紧与源码验收（历史切片）

- 基线 `ef336e448ef1f5eb7228bd446ea7c42580fb853f`，继续原工作分支；既有未跟踪文件全部保护。
- 用 Windows 正式 QML / Controller、隔离 INI 与合成档案复现：长手撕题面挤走运行入口；连接编辑后主动作不在表单可视区域；未提交面试回答时允许切换档案。
- 最小改动：固定手撕操作行、题面/代码切换、现有 Grader 统一按钮与快捷键入口；按本轮结果显示失败/通过和 revision；连接编辑定位与就绪状态；复盘状态文案；把 Profile ID 纳入编辑器身份，并拒绝带未提交草稿切档。
- 不更改动态阶段、Catalog、Workspace、Event、评分或 Provider 架构；不扩展官方答案、STT/TTS，不打包、全量测试或触发 CI。
- 证据：`workspace/maintainer/ux-20260907/`（ignored）；布局覆盖 900×620、1080×680、1280×800、1440×900，浅色 100% / 深色 125%；按钮与 Ctrl+R 执行真实本地 Grader。
- 结束条件：上述定向验证通过、亲自查看截图、记录实际启动路径与风险，然后交给用户手工验收。真实云模型和 macOS 本轮不重复验证。

## 2026-09-05：整场动态面试验证切片（当前执行）

用户本轮明确要求真实多轮测试与修改；以下内容替代旧计划仅验证首问的范围，其余历史记录保留。

- 起点 `972e85c`，分支 `fix/dynamic-interview-full-flow-20260905`；保护所有既有未跟踪资料。不运行全量、CI、打包或发布。
- 已知缺口：后续请求没有完整历史/岗位技能正文；没有经历→八股→本地手撕的阶段推进；UI 只能选一份材料；动态结果沿用固定题权重，不能作为完整报告。
- 目标：本地开场 → AI 根据合成简历/JD 与实际回答深挖、追问 → 岗位原理 → 选择本地可验证手撕 → 真实 Grader → 证据报告。只冻结已经展示的题目，不生成未来题单。
- 范围：复用现有 Session/材料/Provider/Codex/Grader；补齐小型阶段规则、逐轮上下文、单次结果提交，以及正式面试页的材料和状态入口。模型和推理强度使用现有配置，验证真实传参；不新增 Provider。
- 手撕边界：模型从本地就绪候选中选择 ID，不生成未验证代码题或答案；无候选时明确缺失，不伪装完整面试。AI 主观评分和本地代码证据继续分离，不授予 mastery。
- 验证数据：全新 ignored `workspace/maintainer/dynamic-full-20260905/`、合成 Profile/简历/JD；不读取真实 UAT 材料、档案或 Secret。真实 Codex 使用已安装兼容 CLI，仅设置本次隔离应用。
- 顺序：定向复现缺口 → 核心/界面局部修改 → Fake 多轮及权限/传参验证 → Windows 正式页面 + 真实 Codex 完整链路 → 截图人工检查 → 记录实际结果和未验证项。
- 终止/风险：缺真实 API 凭证时不伪造云验证；网络失败保留答案并报告；不得用 Fake 响应充当真实面试成功。最多保存已发生对话与本地流程要求，不新增动态 Runtime 框架。
- 进度（2026-09-06）：真实 Codex 完成六题，含经历追问、八股追问、已验证本地手撕与报告；重启恢复通过。最终 24 项定向测试通过。模型响应仍慢，普通云 API 未有真实 Key 实测；详见同目录 `luna-max-dynamic-interview-status.zh.md` 最新章节。
- 实现提交：`ec46bba32a475b6ca4217903bc3ef7d44445459a`。当前为 `REAL_CODEX_INTERVIEW_COMPLETED / WAITING_FOR_USER_UAT`，不发布、不扩大为所有 Provider 与所有岗位已验证。
...
## 10. 2026-09-07 用户反馈切片：连接、中文题面与阅读布局

基线：`428efd6`；沿用当前分支，保留现有未跟踪文件与真实数据。

- [x] 删除面试标题旁的“详情”及重复标题；连接与授权收进回答区，保留综合评分。
- [x] 页面自定义 `palette` 改为 `colors`；保留 Qt 控件的原生 `palette`。字体告警仍存在，不屏蔽日志、不宣称已修复。
- [x] Codex 同场、同模型、同授权范围复用线程；换档/换场/材料权限变化仍隔离。真实两轮线程 ID 相同，首轮仍有长等待。
- [x] DeepSeek 使用现有 HTTP/SSE 适配器，提供模型/推理选项，正文与 reasoning_content 分离；Key 仅进系统 Keyring。真实首轮暴露 null 字段问题，补明确 JSON Schema 后跑通。
- [x] 当前 45 份面试手撕题默认中文，英文可选；保留冻结原题与 Grader，不改课程或评分语义。
- [x] 仅目标测试、隔离真实 API 探针和 Windows QML 截图检查；不全量回归、不打包、不 CI、不发布。

验收：提交后下一问不要求手工重连；材料取消后旧线程不能继续使用；DeepSeek 能从连接页选择模型和推理模式；中文题面能切换英文；关键尺寸无文字覆盖。真实远程失败单独记录，不用模拟测试冒充。

实现提交：`0d3f0b24595c6ae6c5c937a68d9f57c2a17c68bc`；最后的回答区控件中线修正为 `29d8eaaeb8ce89a411369149380240b113b72637`。最新命令、真实耗时和剩余限制见 [当前状态报告](luna-max-dynamic-interview-status.zh.md)。保持 `WAITING_FOR_MANUAL_INTERVIEW_UAT`，不进入发布。

## 11. 2026-09-07：AI 仅用于面试、经历深挖与真实手撕选题

基线 `1940add3`。以本轮用户明确要求为准，不调整课程、掌握状态、综合评分、Provider 架构或发布版本。

- [x] 删除正式 CoachPage、导航、练习侧栏/Drawer 和 Codex 教练/仓库代理按钮；历史记录与 CLI 兼容入口不删除。同步 QML 文件清单与文档，不打包。
- [x] 新增实际发送的动态面试策略：先听完整经历，再沿回答逐层追问；不把简历当作已经回答；未知/非本人负责时换角度；区分八个岗位与三档强度。
- [x] 复现 AI 错题号导致卡住；合法建议保留，无效建议按已讨论技能/岗位权重改选本地可运行题，持久化更正并在题面上方说明。无候选时明确保留未完成环节。
- [x] 定向服务测试、生产 QML 交互、合成材料的真实 API 对话与截图逐图检查；先识别第一轮提问堆叠问题，再收紧策略并实测。
- [x] 真实运行发现材料刷新会清空已测试连接状态；只对同档案、同配置保留就绪结果，重新保存（含换 Key）或换档均失效。
- [x] 完成最终目标验证及结果记录；提交源码与文档，不运行完整 pytest、构建或 CI。

保持 `WAITING_FOR_MANUAL_INTERVIEW_UAT`。模型实际措辞存在变动，不能用接口返回成功代替追问质量验收；实测覆盖与限制写入同一状态报告。

## 12. 2026-09-07：旧 UAT 目录无法开始与已保存 Key 复用

基线 `b2402456`；只修复用户本次指出的入口和连接复用问题。

- [x] 检查现有 UAT 的公开资源，不读取真实 Profile/材料正文。确认 `dynamic-interviewer.md` 缺失，旧 marker 仍为同版本 v3；用两个定向测试复现未同步。
- [x] 更新公共资源 revision；源码覆盖目录每次启动同步公开资源，不触碰私人档案。资源缺失错误放到开始按钮附近并记录发生阶段。
- [x] 已保存连接优先展示；编辑 Key 留空保留、新值替换、确认后删除。新增连接不覆盖已有 ID；不回显或明文存储 Key。
- [x] 面试设置页直接复用已保存 Key 检测连接，成功自动打开授权预览；不要求重填或手工绕回连接页。
- [x] 从正式页面点击设置/开始/确认/提交，真实 DeepSeek 两轮通过；另一次进程启动恢复同一合成档案、同一面试并继续下一轮，未重填 Key。
- [x] 单独验证资源升级保留数据、连接编辑/删除、重启正式入口及关键尺寸；不运行全量测试、CI 或构建。

具体命令、耗时和截图在同一状态报告中。用户旧窗口未被强行关闭，须重启以载入更新的源码；不自动进入发布阶段。
