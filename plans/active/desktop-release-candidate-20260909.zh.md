...
> 候选阶段已结束。之后用户明确授权发布 Alpha.4，沿用 `4f93969` 验收产物，增加中文文档与目录整理；[发布流程](../../docs/release-process.zh.md)另行记录。下文“不推送 main／不发布”是候选阶段边界，不覆盖后续授权，也不改变候选测试事实。
...
- 起始源码版本 `0.4.0a3`；当时发布 workflow 写死 alpha.3，正式 publish 有覆盖旧资产行为。本轮已改为 `0.4.0a4`、显式门禁且拒绝覆盖；最终平台状态见下方复盘。
...
1. **六项反馈与基线**：可收起侧栏、移除左下档案装饰（设置保留切换）、宽屏自然布局、完整中英题面、恢复配置与 SHA 绑定的材料选择、启动自动连接。真实 QML 输入/点击/重启与四尺寸双主题字体检查。逐场发送确认保留，记住选择不是启动发送。
...
- [x] 性能基线、草稿恢复及最终五次性能复测；Windows 包内 641 个公共文件 SHA 一致，27 道代码题具备运行环境。
- [x] 更新功能及发布脚本/资源升级定向验证；生产下载器真实下载并核对历史 Alpha.3 包。匿名查询实际遇到限流，错误/官方页面回退可用，没有宣称查到未发布的 Alpha.4。
- [x] 唯一本地完整回归及所有失败逐项复验；Windows 最终包原生启动、保存、脚本、17 项公开测试、提交、重启和旧 synthetic 数据升级。
- [x] 最终 macOS 14 最低声明来源的 CI/Artifact 与同轮全部平台门禁：Run 34288557804 / 4f93969 成功，发布 Job 跳过。
- [x] 最终报告、三个同源 CI 文件及 SHA 校验、候选提交/推送；无 main/Tag/Release 操作，交用户审阅。

## 最终复盘

当前状态：`CANDIDATE_TECHNICAL_GATES_PASSED / WAITING_FOR_MAINTAINER_RELEASE_REVIEW`。

- 六项人工反馈、草稿恢复、知识检索、内容发现、匿名更新与发布门禁已完成。本轮始终保护起点已有改动和未知未跟踪文件；没有读取真实档案/材料/密钥，没有修改旧事件、评分、掌握条件或冻结题目。
- 最终同源交付为 `4f939695ce0d411356f6121ea46e5569018366a2` 的 Windows MSVC ZIP、macOS arm64 app ZIP 和 DMG，均已实际下载核对 SHA。应用代码与本地完整原生验收的 `2729e280` 一致；后续差异仅测试/文档及 macOS 最低系统声明。最终文档提交不改变二进制源码，不能把文档 HEAD 写成包的构建 SHA。
- Windows 候选技术验收通过；macOS arm64 在 macOS 15 Runner 的测试、实际构建/启动、执行 worker、签名与 ZIP/DMG 隐私门禁通过。macOS 14 是完整运行库的最低声明，不是已在 14 用户机器完成验收。
- 候选公开题库仍为 96 ready / 24 retention-ready；包内环境可运行 27 道已验证代码题。NNL-018 两组变式保留 AUTHOR_PENDING_REVIEW，不伪装成人工准入。
- 最终同机五次性能数据：搜索中位数 -40.47%，其余预先冻结阈值均通过；28 张真实 QML Before/After 保留源 SHA。不用截图证明 AI、语音或 Mac 实机可用。
- 唯一本地全量的 51 failed / 2 errors 原始记录保留，失败逐项复验 54 passed；最终远端各核心/桌面 Job 全部成功，没有新增 skip/xfail 或减少门禁。
- 公开发布仍需用户批准；真实模型、麦克风、macOS 用户实机以及 Developer ID/公证不在已通过证据内。已完成可独立验证的候选交付，不继续扩展功能或重复全量。

具体文件路径、SHA、测试数量、截图、性能与更新/回退方式见[最终候选报告](../../docs/desktop-candidate-20260909-report.zh.md)。

### 2026-09-09 08:10 HKT 最终候选封板

- Run 34288557804：Ubuntu 三 Python 各 650 passed / 32 skipped；Windows core 三 Python 各 649 passed / 33 skipped；CPU 2 passed；文档 28 passed；Windows 桌面 83 passed，构建/Artifact 上传成功；macOS 桌面 944 passed / 8 skipped，构建/启动/两 worker/签名/ZIP/挂载 DMG 检查成功。发布 Job 为 skipped。
- Windows Artifact 10082008977、macOS Artifact 10081946619 已实际下载至 `dist/candidate-ci-4f93969/`；外层 GitHub digest、三个内层发行文件 SHA 和包内来源/版本均匹配。每个平台 641 个公共文件 SHA 与各自 manifest 一致；Mac `.gitignore` 为 Git blob LF，本机 checkout CRLF，逐项核对后无源码差异。没有忽略其他不匹配或改写包来通过检查。
- 最终 Windows 下载包用新的中文/空格应用目录和已存在的本轮 `ci-synthetic-msvc` synthetic 数据原生启动；点击“继续练习”后原代码恢复，正常退出，Profile/events/submission 三个 SHA 未变。完整首次建档、公开测试 17 passed 和提交路径来自同应用源码的本地 MinGW 包；没有宣称每一个二进制均重复完整 UAT。
- 本轮原生测试应用均已正常退出，没有结束用户原有应用、清理旧候选或覆盖真实数据。后续发现的 P2 输出尾部“测试版本”命名留待小型文案修订，不触发本轮再次打包。
