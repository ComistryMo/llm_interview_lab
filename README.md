...
连接普通 LLM API 或 Codex 面试官后，桌面端会先展示上下文和问题计划，用户确认后才创建面试；AI 只能生成非代码问题文字，Coding 题仍来自本地已验证题库。Codex 计划通过官方 App Server 的结构化 `outputSchema` 生成，并使用“设置 → 模型与推理强度”中选择的模型和推理强度。非代码回答还可选择“录音 → 停止 → 转录 → 编辑 → 提交”，远程转录需要本场明确授权，失败时可直接改用文字。
...
| 方式 | 适合什么场景 | 能力边界 |
|---|---|---|
| 普通 LLM API | 解释、分级提示、只读审查、个性化面试计划、面试追问与可选语音转录 | 只收到上下文预览中勾选的文本或明确授权的音频，不能操作仓库 |
| Codex | 读取获准文件、生成个性化面试计划、运行测试、流式事件、显示 Diff | 使用官方 App Server；面试计划只生成结构化非代码问题，命令和写文件遵守 Sandbox 与显式审批 |
