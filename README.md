...
连接普通 LLM API 后，桌面端首版 Golden Path（后训练工程师 / 校招 / 标准难度）会先展示上下文和问题计划，用户确认后才创建面试；AI 只能生成非代码问题文字，Coding 题仍来自本地已验证题库。非代码回答还可选择“录音 → 停止 → 转录 → 编辑 → 提交”，远程转录需要本场明确授权，失败时可直接改用文字。
