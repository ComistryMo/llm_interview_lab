...
Flickable {
    ...
    ColumnLayout {
        ...
        Text { text: "看能力证据，不看连续打卡"; color: root.colors.text; font.pixelSize: 24; font.bold: true }
        Text { text: "刷题掌握、间隔复测和模拟面试证据彼此分离，并且可以追溯。"; color: root.colors.muted }
        RowLayout {
            ...
            Repeater {
                ...
                delegate: LabCard {
                    ...
                    Layout.fillWidth: true; Layout.preferredHeight: 132; cardColor: root.colors.surface; borderColor: root.colors.border
                    Text { text: modelData.label; color: root.colors.muted }
                    Text { text: modelData.value; color: root.colors.text; font.pixelSize: 32; font.bold: true }
                }
            }
        }
        LabCard {
            ...
            Text { text: "岗位训练证据"; color: root.colors.text; font.pixelSize: 18; font.bold: true }
            Text { text: "掌握率只评价做过的内容；覆盖率说明当前证据能代表多少岗位技能。"; color: root.colors.muted; font.pixelSize: 12 }
            Repeater {
                ...
                delegate: ColumnLayout {
                    ...
                    RowLayout {
                        Layout.fillWidth: true
                        Text { text: modelData.label; color: root.colors.text; font.bold: true }
                    ...
                    RowLayout {
                        Layout.fillWidth: true
                        Text { text: "已评测掌握"; color: root.colors.muted; font.pixelSize: 10; Layout.preferredWidth: 82 }
                        ...
                        Text {
                            ...
                            color: root.colors.muted; Layout.preferredWidth: readinessEvidence.hasMastery ? 52 : 100
                        }
                    }
                    RowLayout {
                        Layout.fillWidth: true
                        Text { text: "证据覆盖"; color: root.colors.muted; font.pixelSize: 10; Layout.preferredWidth: 82 }
                        ProgressBar {
            ...
            Text { visible: !app.dashboard.role_readiness || app.dashboard.role_readiness.length === 0; text: "首次启动时选择目标岗位，即可生成这个本地视图。"; color: root.colors.muted }
        }
        LabCard {
            Layout.fillWidth: true; Layout.preferredHeight: 140; cardColor: root.colors.surface; borderColor: root.colors.border
            Text { text: "如何理解"; color: root.colors.text; font.bold: true }
            Text { width: parent.width; text: "覆盖不足表示证据或课程资产不足，不表示你的能力为 0。掌握率只描述已评测部分；这些本地证据不是 Offer 概率、录用决定，也不能代替真实面试。"; color: root.colors.muted; wrapMode: Text.Wrap }
        }
    }
}
