...
macos.permissions =
mode = standalone
extra_args = --quiet --assume-yes-for-downloads --noinclude-qt-translations --macos-create-app-bundle --macos-target-arch=arm64 --macos-app-icon=dist/icons/LLMInterviewLab.icns --macos-signed-app-name=io.github.comistrymo.llminterviewlab --macos-app-name=LLMInterviewLab --macos-app-mode=gui --macos-app-version=0.4.0 --macos-app-macos-min-version=12.0 --include-package=llm_interview_lab --include-package=pytest --include-package=keyring --include-package=httpx --nofollow-import-to=any_llm --nofollow-import-to=anthropic --nofollow-import-to=google.genai --nofollow-import-to=ollama --nofollow-import-to=openai --include-data-dir=curriculum=runtime_assets/curriculum --include-data-files=curriculum/problems=runtime_assets/curriculum/problems/=**/*.py --include-data-files=curriculum/retention=runtime_assets/curriculum/retention/=**/*.py --include-data-dir=workspace/schema=runtime_assets/workspace/schema --include-data-dir=workspace/templates=runtime_assets/workspace/templates --include-data-dir=coach=runtime_assets/coach --include-data-files=AGENTS.md=runtime_assets/AGENTS.md --include-data-files=.gitignore=runtime_assets/.gitignore --report=desktop-macos-nuitka-report.xml
