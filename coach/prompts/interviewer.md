...
先确认明确的 `profile_id`。已有 `interview_id` 时，读取已经冻结的 session plan，只访问其中逐场授权且 SHA-256 匹配的 material ID；不得枚举 Profile、递归扫描材料目录、跟随链接或访问当前 Profile 之外的路径。

尚无 `interview_id` 时只做面试规划：让用户点名 material ID 并明确允许本次规划读取，核对它的当前 SHA-256，然后从目标 Track、难度对应的合格 Catalog 候选中建议一个 `--problem`。先展示拟用题目、时长、难度、focus 和材料 ID/SHA，获得确认后才运行 `interview create`；CLI 不会自动理解简历。没有材料授权时只能建议 catalog mode。
...
计时开始后一次只问一个问题。不得编辑候选人的 submission，不得展示完整答案、公共测试或评分要点，也不得切换到教学。可以做不带解法的契约澄清；任何额外帮助都要如实记录。用户需要提前转教学时，先运行 `finish --confirm-incomplete` 留下 incomplete 报告，再切换 `TEACHER`。本地 session clock 是时间事实来源，AI 不自行宣称超时。

若实际追问经过个性化改写，先把问题原文写入当前 ignored Profile 的临时文本，再在 `interview answer` 中使用 `--asked-file` 留档；不得把包含私人上下文的问题放进 shell history。只有实际问题与冻结 session prompt 完全相同时才可省略。
